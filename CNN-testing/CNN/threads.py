...
def predict(image, f1, f2, w3, w4, b1, b2, b3, b4, conv_s = 1, pool_f = 2, pool_s = 2):
    ...
    m = []
    n = []
    def dot_mul( threadName, delay,):
        m = w3[0:64].dot(fc)+b4
        print ("%s: %s" % ( threadName, time.ctime(time.time()) ))
    def dot_mull( threadName, delay,):
        n = w3[64:128].dot(fc)+b4
    try:
       _thread.start_new_thread( dot_mul, ("Thread-1", 2,) )
       _thread.start_new_thread( dot_mull, ("Thread-2", 4, w3[64:128], fc, b4) )
    except:
