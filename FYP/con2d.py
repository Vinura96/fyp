...
model = tf.keras.models.load_model('tx.h5')
...
xx2=tf.keras.backend.relu(
    xx2getsizeof(a), alpha=0.0, max_value=None, threshold=0
)
...
ll=tf.keras.backend.dot(xx4,x)

xx5=tf.keras.backend.bias_add(ll, y)
